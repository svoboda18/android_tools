...
#ifndef SVB_MINGW
static inline int ext2fs_open_file(const char *pathname, int flags, mode_t mode)
{
	if (mode)
...
static inline int ext2fs_stat(const char *path, ext2fs_struct_stat *buf)
{
...
static inline int ext2fs_fstat(int fd, ext2fs_struct_stat *buf)
{
#if defined(HAVE_FSTAT64) && !defined(__OSX_AVAILABLE_BUT_DEPRECATED)
