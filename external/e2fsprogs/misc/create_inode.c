...
#if !defined HAVE_PREAD64 && !defined HAVE_PREAD && !defined HAVE_MMAP
				 ...
				 char *zerobuf)
{
...
#else
    {
        got = end;
        off = start;
		buf = mmap(NULL, end, PROT_READ, MAP_PRIVATE, fd, 0);
#endif
		if (got < 0 || !buf) {
			err = errno;
			goto fail;
			       ...
			       struct fs_ops_callbacks *fs_callbacks)
{
	...
	for (i = 0; i < num_dents; free(dent[i]), i++) {
		...
		if (!S_ISDIR(st.st_mode) && !S_ISLNK(st.st_mode) &&
		    st.st_nlink > 1) {
			hdlink = is_hardlink(hdlinks, st.st_dev, st.st_ino); /* S_ISLNK **/
			if (hdlink >= 0) {
		...
		case S_IFIFO:
#if !defined(_WIN32) || defined(SVB_WIN32)
#if defined(S_ISSOCK) && !defined(SVB_WIN32)
		case S_IFSOCK:
...
find_lnf:
		...
		default:
			com_err(__func__, 0,
				_("ignoring entry \"%s\" (%x)"), name, st.st_mode & S_IFMT);
		}
