...
[[ $DEBUG -eq 1 ]] &&
 exit 0 # target only rel builds

STANDALONE_STRIPPED=true
STRIP_FLAGS=""
...
while [[ $# -gt 0 ]]; do # last arg(s) are/is file(s)
    ...
    case "$1" in
        ...
        *) FILE_NAME="$(basename "$1")"; echo -e "  STRIP     $FILE_NAME"; $STANDALONE_STRIPPED && FILE_NAME="-o ${SRP}/${FILE_NAME%.*}-stripped.${FILE_NAME#*.} $1" || FILE_NAME="$1"; STRIP_FLAGS="${STRIP_FLAGS} $FILE_NAME ";;#eval $TOPDIR/scripts/genpdb.sh "$FILE_NAME
    esac
    shift
done
